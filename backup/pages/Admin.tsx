...
export const Admin: React.FC<AdminProps> = ({ songs, setSongs, settings, setSettings, user }) => {
  ...
  const [manualForm, setManualForm] = useState<Omit<Song, 'id'>>({
    title: '', artist: '', album: '', coverUrl: '', platformUrl: ''
  });
  ...
  const handleAddSong = async () => {
    ...
    try {
      const newSongs = await fetchMusicData(targetInput);
  ...
  const handleManualSubmit = () => {
      ...
      setSongs(prev => [...prev, { ...manualForm, id: Date.now().toString(), coverUrl: manualForm.coverUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(manualForm.title)}&background=27272a&color=fff` }]);
      setShowManualModal(false);
      setManualForm({ title: '', artist: '', album: '', coverUrl: '', platformUrl: '' });
  };
            ...
            {activeTab === 'settings' && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                    ...
                    <section className="space-y-4">
                        <h3 className="text-lg font-bold text-white flex items-center gap-2"><Cookie className="text-amber-500" size={20}/> <span>Cookie 注入</span></h3>
                        <div className="p-1 bg-gradient-to-b from-white/5 to-transparent rounded-2xl border border-white/10">
                            <div className="bg-zinc-900/80 rounded-xl p-4 space-y-4 backdrop-blur-sm">
                                <div className="flex items-start gap-3">
                                    <ShieldAlert className="text-zinc-500 shrink-0 mt-1" size={16} />
                                    <div className="space-y-1"><p className="text-sm font-medium text-zinc-300">身份凭证</p><p className="text-[11px] text-zinc-500">粘贴 Cookie 以获取高音质。</p></div>
                                </div>
                                <textarea value={settings.cookie || ''} onChange={e => setSettings({...settings, cookie: e.target.value})} className="w-full h-32 bg-black/50 border border-zinc-700 rounded-xl p-4 text-xs font-mono text-amber-200/80 focus:ring-1 focus:ring-amber-500/50 outline-none resize-none placeholder-zinc-700 custom-scrollbar" placeholder="例如: MUSIC_U=...;" spellCheck={false}/>
                            </div>
                        </div>
                    </section>
        ...
        {showManualModal && (
             <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 w-full max-w-md space-y-4">
                    <h3 className="text-lg font-bold text-white">手动添加</h3>
                    <input className="w-full bg-black/50 border border-zinc-700 rounded-lg p-3 text-white" placeholder="标题" value={manualForm.title} onChange={e => setManualForm({...manualForm, title: e.target.value})}/>
                    <input className="w-full bg-black/50 border border-zinc-700 rounded-lg p-3 text-white" placeholder="歌手" value={manualForm.artist} onChange={e => setManualForm({...manualForm, artist: e.target.value})}/>
                    <input className="w-full bg-black/50 border border-zinc-700 rounded-lg p-3 text-white" placeholder="封面 URL" value={manualForm.coverUrl} onChange={e => setManualForm({...manualForm, coverUrl: e.target.value})}/>
                    <div className="flex gap-3"><button onClick={() => setShowManualModal(false)} className="flex-1 py-3 rounded-xl border border-zinc-700 text-zinc-400">取消</button><button onClick={handleManualSubmit} className="flex-1 py-3 rounded-xl bg-white text-black font-bold">确认</button></div>
                </div>
            </div>
        )}
      </div>
