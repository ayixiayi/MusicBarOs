...
export const AudioSpectrum: React.FC<AudioSpectrumProps> = ({ audioRef }) => {
  ...
  const startAudio = async () => {
    try {
      ...
      // [关键优化] 强制高音质回放模式
      // latencyHint: 'playback' 允许更大的缓冲区，防止爆音
      // sampleRate: 48000 匹配大多数 Android 设备原生采样率，避免重采样劣化
      ...
      // 如果提供了 audioRef，直接连接 audio 元素
      if (audioRef && audioRef.current) {
        console.log("[Spectrum] Connecting to audio element source", audioRef.current.src);
        try {
           sourceNode = ctx.createMediaElementSource(audioRef.current);
        } catch (e) {
           console.warn("[Spectrum] Element already connected or invalid, reusing/ignoring:", e);
           return; 
        }
      } else {
        // [音质保护] 彻底移除麦克风回退逻辑
        // 任何对 getUserMedia({audio: true}) 的调用都可能触发 Communication Mode 导致音质暴跌
        console.warn("[Spectrum] No audio ref provided. Visualization disabled to protect audio quality.");
        return;
      }
      ...
      if (sourceNode) {
          ...
          // 必须连接到目的地，否则听不到声音 (MediaElementSource 特性)
          if (audioRef && audioRef.current) {
            console.log("[Spectrum] Connecting analyzer to destination (Speaker)");
      ...
      setErrorMessage("Audio Initialize Failed");
    }
  };
